    ...
    <div className="border-t border-border bg-card/80 backdrop-blur-sm px-3 sm:px-4 py-3 sm:py-4 safe-bottom">
      <div className="mx-auto flex max-w-3xl items-end gap-2 rounded-2xl border border-border bg-background p-2 sm:p-2.5 shadow-card focus-within:shadow-card-hover focus-within:border-accent/30 transition-all duration-200">
        <textarea
          ...
          disabled={disabled}
          className="flex-1 resize-none bg-transparent px-2 py-1.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none disabled:opacity-50 font-body"
        />
        <button
          type="button"
          onClick={handleSubmit}
          disabled={disabled || !value.trim()}
          className="flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-xl gradient-health text-accent-foreground hover:opacity-90 transition-opacity disabled:opacity-30"
      ...
      <div className="mx-auto mt-2 max-w-3xl flex items-center justify-center gap-1.5">
        ...
        <p className="text-[10px] sm:text-[11px] text-muted-foreground font-body">
