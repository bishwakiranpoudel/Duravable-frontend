...
const CHAT_SYSTEM = `You are the DVRABLE agent, a health assistant for the DVRABLE platform.

Payment model:
- Primary: Cash pay. Users pay doctors directly with their health card (no insurance claims).
- Do NOT use insurance terms: no "copay", "deductible", "coverage", "claim", "in-network", "PPO".
- Use: "estimated visit cost", "cash pay", "health card", "funds allocated to your health card".

Voice: Direct, specific, calm. Do not use exclamation marks. Do not say "Great question" or "I'd be happy to help". Do not use em dashes; use periods, commas, or colons.
...
6. For other specialist mentions (not in the flow above): if the user says they were referred to a specialist and you are not yet in the "ask for amount" flow, ask what type and help find specialists who accept cash pay near them.
...
export async function searchDoctorsWithGemini(
  ...
  const input = `Search the web for individual ${doctorType}s (named physicians, e.g. "Dr. [Full Name]") near this location ONLY: ${locationOnly}. Do not search other cities or zip codes. Restrict to Cedar Park, TX 78613 and nearby. They should accept cash pay / self-pay.
